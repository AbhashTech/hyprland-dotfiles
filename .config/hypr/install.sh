...
for app_icon in ocr-language-manager.png permission-manager.png; do
    if [ -f "${CONFIG_DIR}/assets/${app_icon}" ]; then
        mkdir -p "${HOME}/.local/share/icons/hicolor/512x512/apps" "${HOME}/.local/share/icons"
        cp "${CONFIG_DIR}/assets/${app_icon}" "${HOME}/.local/share/icons/"
        for size in 16 24 32 48 64 128 256 512; do
            mkdir -p "${HOME}/.local/share/icons/hicolor/${size}x${size}/apps"
            if command -v magick >/dev/null 2>&1; then
                magick "${CONFIG_DIR}/assets/${app_icon}" -resize "${size}x${size}" "${HOME}/.local/share/icons/hicolor/${size}x${size}/apps/${app_icon}" 2>/dev/null || true
            else
                cp "${CONFIG_DIR}/assets/${app_icon}" "${HOME}/.local/share/icons/hicolor/${size}x${size}/apps/${app_icon}" 2>/dev/null || true
            fi
        done
    fi
done
